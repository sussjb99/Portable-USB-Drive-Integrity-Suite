#!/usr/bin/env pwsh
# Test 4: Repair two missing files using PAR 2.0 data
...
try {
    ...
    Write-Banner "Repairing two missing files using PAR 2.0 data"

    Copy-Item "test-0.data" "test-0.data.orig"
    Copy-Item "test-5.data" "test-5.data.orig"
    Remove-Item "test-0.data"
    Remove-Item "test-5.data"
    ...
    if (-not (Compare-Files "test-5.data" "test-5.data.orig")) {
        Exit-TestWithError "Repaired file test-5.data does not match original"
    }
